...
compute_shap.py
====================
SHAP analysis for FGA 3-class heart murmur model.
...
Outputs (in SHAP_Figures_3Class/):
  ...
  4. Waterfalls/Absent/*.png   - Waterfall per correctly predicted Absent sample
  5. Waterfalls/Present/*.png  - Waterfall per correctly predicted Present sample
  6. Waterfalls/Unknown/*.png  - Waterfall per correctly predicted Unknown sample
"""
import json
...
MAX_PER_CLASS   = 100    # Cap correctly predicted test samples per class.
...
def choose_representative_recording(patient_id, patient_pred, patient_indices, index_to_prob):
    """Pick the recording that contributes most to the patient-level decision."""
...
def plot_signed_bar_chart(class_imp_signed):
    ...
    ax.set_title(
        'SHAP Signed Feature Importance - Direction of Evidence per Class\n'
        '(+) = Supports prediction   |   (-) = Contradicts prediction',
...
def main():
    ...
    # Step 1: Select patient-level correct cases, then one representative recording.
    print("\nSelecting patient-level correct cases and representative recordings...")
    patient_aggregation_method, patient_threshold = load_patient_aggregation_config()
    print(
        f"  Patient aggregation for SHAP selection: "
    ...
    for ci in range(3):
        print(f"  {CLASS_NAMES[ci]}: {len(selected_per_class[ci])} patient-level correct cases selected")
    ...
    pd.DataFrame(selected_rows).to_csv(
        os.path.join(OUT_DIR, "shap_selected_patient_representatives.csv"),
        index=False,
    )
    ds.norm_stats = load_fold_norm_stats(EXPLAIN_FOLD)
    ...
    for ci in range(3):
        ...
        print(f"\nSHAP for {cname}: {len(cases)} patient-level representative recordings")
        ...
        for idx, pred_label, patient_id in tqdm(cases, desc=f"SHAP {cname}"):
            ...
            if isinstance(shap_values, np.ndarray):
                ...
                # Handle ensemble and older SHAP return formats.
                sv_raw = shap_values[pred_label]
            ...
            print(f"  [{cname}] patient={patient_id} recording={rec_name} done")
